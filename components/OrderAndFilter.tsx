...
const Filter = () => {
  ...
  const handleFilterByType = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedOptions.length >= 2)
      return alert("You cannot filter by more than two types");
  ...
  const handleDeleteFilter = (typeToRemove: String) => {
    const updateOptions = selectedOptions.filter(
    ...
    <aside className="hidden fixed md:flex items-center justify-center left-10 w-10 h-screen hover:w-[25%] group">
      <div className="block group-hover:rotate-180 transition duration-150">
        <svg
          ...
          viewBox="0 0 24 24"
          strokeWidth="1.5"
          stroke={`${darkTheme ? "#18181b" : "#dc2626"}`}
          fill="none"
          strokeLinecap="round"
      ...
      <div
        className={`${
          darkTheme ? "bg-zinc-800" : "bg-red-700"
        } rounded-lg shadow-md shadow-zinc-900 h-[60%] w-[90%] hidden group-hover:block`}
      >
        <div className="flex flex-col gap-2 py-2 px-4">
          ...
          <select
            name="Defense"
            id="Defense"
            onChange={(event) => handleOrderByDefense(event)}
            className="rounded-md py-1"
          >
          ...
          <select
            name="Attack"
            id="Attack"
            onChange={(event) => handleOrderByAttack(event)}
            className="rounded-md py-1"
          >
        ...
        <div className="flex flex-row gap-2 py-2 px-4">
          {selectedOptions.length > 0 &&
            ...
            selectedOptions.map((type, index) => {
              return (
                <span
                  className={`${type} text-white font-bold flex gap-1 rounded-3xl py-2 px-4 border-2`}
                  key={index}
                >
                  {type}
                  <span
                    className="relative font-normal left-1 pb-1 px-1 cursor-pointer"
