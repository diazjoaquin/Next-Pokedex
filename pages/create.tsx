...
const validation = (form : any) => {
    ...
    Number(form.hp) < 0 || Number(form.hp) > 400 ? error.hp = "Enter a valid number between 1 and 400." : "";
    
    // attack validations:

    !form.attack ? error.attack = "Enter a valid number between 1 and 350." : "";
    Number(form.attack) < 0 || Number(form.attack) > 350 ? error.attack = "Enter a valid number between 1 and 350." : "";


    //defense validations:

    !form.defense ? error.defense = "Enter a valid number between 1 and 400." : "";
    Number(form.defense) < 0 || Number(form.defense) > 400 ? error.defense = "Enter a valid number between 1 and 400." : "";


    // speed validations:

    !form.speed ? error.speed = "Enter a valid number between 1 and 100." : "";
    Number(form.speed) < 0 || Number(form.speed) > 100 ? error.speed = "Enter a valid number between 1 and 100." : "";


    // height validations:

    !form.height ? error.height = "Enter a valid number between 1 and 150." : "";
    Number(form.height) < 0 || Number(form.height) > 150 ? error.height = "Enter a valid number between 1 and 150." : "";


    // weight validations:

    !form.weight ? error.weight = "Enter a valid number between 1 and 300." : "";
    Number(form.weight) < 0 || Number(form.weight) > 300 ? error.weight = "Enter a valid number between 1 and 300." : "";

    // image validations: 

    form.image !== "" && !(/\.(jpg|jpeg|png|gif)$/i.test(form.image)) ? error.image = "Please enter a valid URL" : "";
...
const Create = () => {
    ...
    return (
        <main className="w-full h-full flex items-center justify-center">
            ...
            shadow-md`}>
                <div className="pt-2">
                    <h1 className="text-white text-2xl font-bold capitalize mt-4 mb-3">Create your own pokemon</h1>
                </div>
                <form className="lg:w-9/12 w-11/12 h-3/4 grid grid-cols-2 gap-10"> 
                    <div className="flex flex-col w-full gap-2">
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">Name</label>
                            <input className="rounded-md px-2" type="text" name="name" value={form.name} onChange={handleChange} />
                            <div className="h-[20px]">
                                {error.name && <span className="text-xs text-white font-bold h-[20px]">{error.name}</span>}
                            </div>
                        </section>
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">HP</label>
                        ...
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">Attack</label>
                        ...
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">Defense</label>
                        ...
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">Speed</label>
                    ...
                    <div className="flex flex-col w-full gap-2">
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">Height</label>
                        ...
                        <section className="flex flex-col md:gap-1 justify-between">
                            <label className="text-white text-sm after:content-['*'] after:ml-0.5">Weight</label>
                ...
                <button className="text-white rounded-md bg-sky-500 mb-4 py-2 w-4/12 hover:brightness-110" onClick={() => handleSubmit()}>Create pokemon</button>
            </section>
        </main>
