  ...
  return (
    <main className="w-screen h-screen flex items-center justify-center">
      ...
      {isLoading ? (
        ...
        <section
          className={`${darkTheme ? "bg-zinc-800" : "bg-red-600"} 
			w-10/12
			md:w-6/12
			md:h-4/6
			h-5/6
        ...
        >
          ...
          <section className="h-4/5 flex flex-col gap-10">
            <article className="flex justify-center items-center h-3/6">
              <img
                className="w-auto h-auto max-h-full content-center"
                src={data?.imgUrl}
                alt="pokemon"
              />
            </article>
            <article className="flex flex-col gap-14 md:gap-8 h-2/5">
              <div className="flex flex-wrap gap-3 md:gap-4 lg:gap-8  items-center justify-center">
                <span className="text-white border-2 px-3 rounded-full py-2">
                  {data?.hp}
                </span>
                <span className="text-white border-2 px-3 rounded-full py-2">
                  {data?.attack}
              ...
              <div className="flex justify-center items-center gap-4 md:gap-12">
                {data?.types?.map((type: any) => {
                  return (
                    <span
                      key={type}
                      className={`${type} flex text-white capitalize text-sm items-center justify-center gap-2 w-[45%] md:w-[30%] rounded-full py-2`}
                    >
                      <img
                        src={`/images/types/${type}.svg`}
                        className="w-auto h-auto max-w-[30px] "
                        alt="type"
                      />
                      {type}
                    </span>
              ...
              <div
                className={`${
                  data.custom ? "" : "hidden"
                } text-white font-bold text-lg absolute top-3 right-10 capitalize`}
              >
              ...
              >
                <button
                  ...
                  className="text-white text-lg font-bold border-2 px-6 py-1 rounded-lg bg-blue-500"
                >
                  Delete
                </button>
              </div>
